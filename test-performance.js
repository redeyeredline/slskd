...
function testVirtualScrolling(datasetSize) {
  console.log(`📊 Testing Virtual Scrolling with ${datasetSize.toLocaleString()} items...`);
  
  const startTime = Date.now();
  
  // Simulate virtual scrolling - only render visible items
  const visibleItems = Math.ceil(TEST_CONFIG.virtualListHeight / TEST_CONFIG.itemHeight);
...
function testTraditionalRendering(datasetSize) {
  console.log(`📊 Testing Traditional Rendering with ${datasetSize.toLocaleString()} items...`);
...
function runPerformanceTests() {
  ...
  const testSizes = [TEST_CONFIG.smallDataset, TEST_CONFIG.mediumDataset, TEST_CONFIG.largeDataset];
  ...
  testSizes.forEach(size => {
    ...
    console.log(`   Memory Usage: ${(virtualResults.memoryUsage / 1024 / 1024).toFixed(2)}MB`);
    ...
    console.log(`   DOM Nodes: ${traditionalResults.domNodes.toLocaleString()}`);
    console.log(`   Memory Usage: ${(traditionalResults.memoryUsage / 1024 / 1024).toFixed(2)}MB`);
    
    // Calculate improvements
    const memoryImprovement = ((traditionalResults.memoryUsage - virtualResults.memoryUsage) / traditionalResults.memoryUsage * 100).toFixed(1);
    const domImprovement = ((traditionalResults.domNodes - virtualResults.domNodes) / traditionalResults.domNodes * 100).toFixed(1);
...
function testPagination() {
  ...
  pageSizes.forEach(pageSize => {
    ...
    console.log(`   Memory per Page: ${(pageSize * 1024 / 1024 / 1024).toFixed(3)}MB\n`);
  });
}
...
function testSearchPerformance() {
  ...
  searchTerms.forEach(term => {
    ...
    console.log(`   Matches: ${filteredSize.toLocaleString()} items (${matchPercentage.toFixed(1)}%)`);
    console.log(`   Search Time: ${searchTime.toFixed(1)}ms`);
    console.log(`   Memory Usage: ${(filteredSize * 1024 / 1024 / 1024).toFixed(3)}MB\n`);
  });
}

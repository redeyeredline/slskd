...
export function filterTree(tree, filter) {
  ...
  function filterNode(node) {
    ...
    let filteredChildren = children.map(filterNode).filter(Boolean);
    // Check if any file in this folder matches
    let files = node.files || [];
    const fileMatch = files.some(f => ((f.filename || f.name || '').toLowerCase().includes(term)));
...
const DirectoryTree = React.forwardRef(({ onSelect, selectedDirectoryName, tree, username, filter = "" }, ref) => {
  const [listHeight, setListHeight] = useState(500);
  const [collapsedPaths, setCollapsedPaths] = useState(new Set());
  const [controlBarHeight, setControlBarHeight] = useState(0);
  const [treeState, setTreeState] = useState(tree);
  const [prevCollapsedPaths, setPrevCollapsedPaths] = useState(new Set());
  ...
  const filteredTree = useMemo(() => filterTree(treeState || [], filter), [treeState, filter]);
